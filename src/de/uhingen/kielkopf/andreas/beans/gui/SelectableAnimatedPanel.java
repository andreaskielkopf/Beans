            ...
            if (getShadow() instanceof final JLabel sdw)
               synchronized (sdw) {
                  for (final Entry<T, AnimatedPanel<T>.Pair<Long, Point>> entry:allItems.entrySet())
                     if (entry.getValue().getB() instanceof final Point b)
                        if (p.y >= b.y && p.y <= b.y + rowHeight && p.x >= b.x)
                           if (!(treffer instanceof final Entry<T, AnimatedPanel<T>.Pair<Long, Point>> tr)
                              ...
                              if (entry.getKey() instanceof final T key) {
                                 sdw.setText(key instanceof final hasName hn ? hn.getName() : key.toString());
                                 if (p.x < b.x + sdw.getPreferredSize().width)
                                    treffer=entry;
                              }
               }
            if (treffer instanceof final Entry<T, AnimatedPanel<T>.Pair<Long, Point>> tr
               ...
               e.consume();
               repaint(100);
               // System.out.println( (key instanceof final hasName hn ? hn.getName() : key.toString()) + " @ " + p.x + ":" + p.y);
            }
         }
      });
   }
